...
export default async function SearchPage({ searchParams }: SearchPageProps) {
  ...
  return (
    <>
      <Navbar />
      <main className="min-h-screen bg-[#FFFFFF] px-4 pb-16 pt-36 sm:px-6 lg:pt-44">
        <section className="mx-auto max-w-5xl">
          <div className="mb-8">
            <p className="text-xs font-black uppercase tracking-[0.16em] text-[#0B6232]">Search</p>
          ...
          <form action="/search" className="mb-8 flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-3 shadow-lg shadow-slate-900/5 sm:flex-row">
            <label className="relative flex-1">
              ...
              <input
                ...
                className="h-12 w-full rounded-lg border border-slate-200 bg-slate-50 pl-11 pr-4 text-sm font-medium text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-[#0B6232] focus:bg-white"
              />
            </label>
            <button className="h-12 rounded-lg bg-[#0B6232] px-6 text-sm font-black text-white transition hover:bg-[#0B6232]" type="submit">
              Search
            </button>
          </form>
          ...
          <div className="grid gap-3">
            {results.map((result) => (
              <Link
                ...
                href={result.href}
                className="group rounded-xl border border-slate-200 bg-white p-4 shadow-lg shadow-slate-900/5 transition hover:-translate-y-0.5 hover:border-[#FFC66B] hover:shadow-xl hover:shadow-slate-900/10"
              >
